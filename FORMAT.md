...
**格式：`GitHub用户名-领域-序号`**。GitHub 用户名全球唯一，保证不撞。

| 来源 | 格式 | 示例 |
|------|------|------|
| **Polaris 自动生成** | `LP` + 序号 | `LP27`（Polaris 内部唯一） |
| **自主研究** | `用户名-领域-序号` | `val1813-CM-001`、`zhangsan-Q-003` |
| **示范** | `demo-领域-年份` | `demo-SR-1905` |

**为什么必须带用户名：** 两个人同时研究凝聚态，各自叫 `CM-007` → 撞了。`val1813-CM-007` 和 `zhangsan-CM-007` → 永不撞。
